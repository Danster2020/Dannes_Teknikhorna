                ...
                <div className="flex justify-center">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        {posts.map(({ node }, index) => (
                            <Link to={"/blog/" + node.frontmatter.slug} className="max-w-md mx-10 md:mx-0">
                                <div key={node.id} className="flex flex-col bg-[#2D8FA9] bg-opacity-10 p-4 border-2 border-gray-700 rounded-2xl">
                                    <h2 className="mb-4 text-md md:text-lg lg:text-xl text-[#DDF5FD] font-bold">{node.frontmatter.title}</h2>
